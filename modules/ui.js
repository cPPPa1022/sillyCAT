...
import { hasCastCache, startPolling, stopPolling, getScannerStatus } from "./scanner.js";
...
var CAT = CAT_THEMES["奶糖小猫"];
var TEXT_THEMES = { "默认": { font: "inherit", lineHeight: "1.6", color: "inherit", bg: "transparent", fontSize: "inherit" }, "暖纸阅读": { font: "Noto Serif SC, SimSun, serif", lineHeight: "2.0", color: "#4a3728", bg: "#fef9f2", fontSize: "15px" }, "深夜模式": { font: "inherit", lineHeight: "1.7", color: "#d0d0d0", bg: "#1a1a1e", fontSize: "14px" }, "紧凑模式": { font: "Noto Sans SC, sans-serif", lineHeight: "1.4", color: "#1a1a1a", bg: "transparent", fontSize: "13px" } };

function applyTheme(n) { if(typeof CAT_THEMES==="undefined")return; CAT = CAT_THEMES[n] || CAT_THEMES["奶糖小猫"]; if(!CAT) CAT = CAT_THEMES["奶糖小猫"]; var r=document.documentElement; r.style.setProperty("--sl-accent",CAT.accent); r.style.setProperty("--sl-accent-hover",CAT.accentHover||CAT.accent); r.style.setProperty("--sl-line",CAT.line); r.style.setProperty("--sl-card",CAT.card); r.style.setProperty("--sl-radius",CAT.radius); r.style.setProperty("--sl-border",CAT.borderStyle+" "+CAT.line); r.style.setProperty("--sl-shadow",CAT.shadow); r.style.setProperty("--sl-font",CAT.font); r.style.setProperty("--sl-bg",CAT.bg); r.style.setProperty("--sl-text",CAT.text); if(CAT.backdropFilter&&CAT.backdropFilter!=="none")r.style.setProperty("--sl-backdrop",CAT.backdropFilter);else r.style.removeProperty("--sl-backdrop"); syncThemeToDOM(); }


function injectThemeStyle() { var id="sl_theme_overrides"; var el=document.getElementById(id); if(!el){el=document.createElement("style");el.id=id;document.head.appendChild(el);} var t=document.getElementById("sl_global_style"); var rules=""; if(settings.uiTheme==="贴纸小猫"){ rules+="#sl_compact button:active,#sl_panel button:active{transform:translate(2px,2px)!important;box-shadow:none!important}"; } else if(settings.uiTheme==="像素小猫"){ rules+="#sl_compact button,#sl_panel button{transition:none!important}#sl_compact button:hover,#sl_panel button:hover{filter:none!important;transform:none!important}"; } else if(settings.uiTheme==="云朵猫窝"){ rules+="#sl_panel{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}"; } el.textContent=rules; }
function syncThemeToDOM() { if(_compactEl&&_compactEl.length) { _compactEl.css({background:CAT.card,borderRadius:CAT.radius,boxShadow:CAT.shadow,border:CAT.borderStyle+" "+CAT.line,fontFamily:CAT.font,color:CAT.text}); _compactEl.find("#sl_comp_header").css({borderBottomColor:CAT.line}); _compactEl.find("#sl_comp_scan").css({background:CAT.accent}); _compactEl.find("#sl_comp_gen").css({background:CAT.accent,borderRadius:CAT.borderRadiusBtn,boxShadow:CAT.btnShadow}); _compactEl.find("#sl_comp_auto").css({borderColor:CAT.line,color:CAT.text,borderRadius:CAT.borderRadiusBtn}); } if(_panelEl&&_panelEl.length) { _panelEl.css({background:CAT.bg,borderRadius:CAT.radius,boxShadow:("0 4px 32px "+CAT.shadow.split("rgba")[1]||"rgba(0,0,0,0.2)"),border:CAT.borderStyle+" "+CAT.line,fontFamily:CAT.font,color:CAT.text}); _panelEl.find("#sl_panel_header").css({borderBottomColor:CAT.line}); _panelEl.find(".sl_tab_btn.active").css({color:CAT.accent,borderTopColor:CAT.accent}); _panelEl.find("#sl_panel_tabs").css({borderTopColor:CAT.line}); } }
...
function sbtn(c) { return "padding:" + (CAT.btnPadding||"8px 16px") + ";border:none;border-radius:" + (CAT.borderRadiusBtn||CAT.radius) + ";background:" + c + ";color:#fff;cursor:pointer;font-size:12px;font-weight:600;box-shadow:" + (CAT.btnShadow||"none") + ";transition:all 0.2s cubic-bezier(0.34,1.56,0.64,1);"; }
function fi() { return "width:100%;padding:" + (CAT.inputPadding||"8px 10px") + ";border:" + CAT.borderStyle + " " + CAT.line + ";border-radius:" + (CAT.borderRadiusInput||"8px") + ";background:" + CAT.card + ";font-size:12px;color:" + CAT.text + ";box-sizing:border-box;outline:none;transition:all 0.2s ease;"; }
function chkProf() { try { var s = JSON.parse(localStorage.sillab_settings); var pf = s.profiles || {}; var cn = Object.keys(pf)[0]; if (!cn || !pf[cn]) return false; var cast = pf[cn].cast || {}; var keys = Object.keys(cast); if (keys.length === 0) return !!(pf[cn].meta && pf[cn].meta.cardType === '世界观卡'); for (var i = 0; i < keys.length; i++) { if (cast[keys[i]] && cast[keys[i]].static && cast[keys[i]].static.length > 10) return true; } return false; } catch (e) { return false; } }
...
function showPanel() { if (!_panelEl) createExpandedPanel(); _expandedVisible = true; CAT = CAT_THEMES[settings.uiTheme || '奶糖小猫']; _panelEl.css({ left: "50%", top: "50%", right: "auto", bottom: "auto", transform: "translate(-50%,-50%)", maxHeight: "80vh", height: "80vh", width: "540px", flexDirection: "column" }); _panelEl.fadeIn(200, function () { _panelEl.css("display", "flex"); }); if (_compactEl) _compactEl.hide(); renderHomeTab(); }
function hidePanel() { _expandedVisible = false; if (_panelEl) _panelEl.hide(); if (_compactEl) _compactEl.show(); refreshCompactBar(); }
...
function renderHomeTab() {
  ...
  body.off("change", "#sl_ui_theme").on("change", "#sl_ui_theme", function () { var name = jQuery(this).val(); hidePanel(); settings.uiTheme = name; applyTheme(name); saveSettings(); if (_compactEl) { clearInterval(_compactTimer); _compactEl.remove(); _compactEl = null; createCompactBar(); } showPanel(); });
...
function bindSettingsEvents(b) {
  b.off("change", "#sl_plugin_on").on("change", "#sl_plugin_on", function () { settings.pluginOn = jQuery(this).is(":checked"); saveSettings(); if (settings.pluginOn) { startPolling(); if (_compactEl) _compactEl.show(); if (!_compactTimer) { _compactTimer = setInterval(refreshCompactBar, 3000); } } else { stopPolling(); if (_compactEl) _compactEl.hide(); if (_compactTimer) { clearInterval(_compactTimer); _compactTimer = null; } } });
...
function bindArchiveEvents(b) {
  b.off("click", "#sl_btn_clear_cast").on("click", "#sl_btn_clear_cast", function(){if(!confirm("确定要清除当前角色卡档案喵？聊天记录不受影响~"))return;deleteCharacterProfile(false);toast("warning","档案清除啦~ 需要重新扫描喵 (｡•́︿•̀｡)");renderArchiveTab();refreshCompactBar();});
  b.off("click", "#sl_btn_scan_cast3").on("click", "#sl_btn_scan_cast3", async function () { var btn = jQuery(this); btn.prop("disabled", true).text("扫描中..."); try { var scanMode = jQuery('#sl_scan_mode').val() || 'zit'; await scanCharacterProfile(scanMode); toast("success", "扫描完成喵~ 🔒 已锁定"); renderArchiveTab(); refreshCompactBar(); } catch (e) { toast("error", "扫描失败喵…"); } btn.prop("disabled", false).text("🔍 扫描角色卡档案喵~"); });
...
export function buildUI(extSettings) { slLog("=^_^= UI v2.1 start"); var knownThemes=["奶糖小猫","贴纸小猫","云朵猫窝","像素小猫"]; if(settings.uiTheme && knownThemes.indexOf(settings.uiTheme)===-1){ settings.uiTheme="奶糖小猫"; try{saveSettings();}catch(e){} } applyTheme(settings.uiTheme||"奶糖小猫"); if(!document.getElementById("sl_global_style")){var s=document.createElement("style");s.id="sl_global_style";s.textContent="#sl_compact,#sl_panel{font-family:var(--sl-font)!important}#sl_compact button,#sl_panel button{transition:all 0.2s cubic-bezier(0.34,1.56,0.64,1)!important}#sl_compact button:hover,#sl_panel button:hover{filter:brightness(0.92);transform:translateY(-1px)}#sl_compact button:active,#sl_panel button:active{transform:translateY(0) scale(0.97)}#sl_panel input,#sl_panel select,#sl_panel textarea{border-radius:var(--sl-radius)!important;border:var(--sl-border)!important;background:var(--sl-card)!important;color:var(--sl-text)!important}#sl_panel input:focus,#sl_panel select:focus,#sl_panel textarea:focus{border-color:var(--sl-accent)!important;box-shadow:0 0 0 3px rgba(99,102,241,0.1)!important}#sl_panel .sl_tab_btn{transition:all 0.2s ease!important}";document.head.appendChild(s);} createCompactBar(); createExpandedPanel(); injectThemeStyle(); }
