...
function bindArchiveEvents(b) {
  ...
  b.off("click", "[id^=sl_port_]").on("click", "[id^=sl_port_]", function () { var bg = jQuery(this).css("background-image"); if (!bg || bg === "none") return; var url = bg.replace(/^url\\([\"']?/, "").replace(/[\"']?\\)$/, ""); jQuery('<div style="position:fixed;z-index:30001;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;cursor:pointer;"><img src="' + url + '" style="max-width:90vw;max-height:90vh;border-radius:12px;box-shadow:0 4px 30px rgba(0,0,0,0.5);"></div>').appendTo("body").on("click", function () { jQuery(this).remove(); }); });
}
